...
func (ec *executionContext) resolveEntity(
	...
	case "AnimeNews":
		resolverName, err := entityResolverNameForAnimeNews(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "AnimeNews": %w`, err)
		}
		switch resolverName {
		...
		case "findAnimeNewsByID":
			id0, err := ec.unmarshalNID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findAnimeNewsByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindAnimeNewsByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "AnimeNews": %w`, err)
			}
...
func entityResolverNameForAnimeNews(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for AnimeNews", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for AnimeNews", ErrTypeNotFound))
			break
		}
		return "findAnimeNewsByID", nil
	}
	return "", fmt.Errorf("%w for AnimeNews due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
